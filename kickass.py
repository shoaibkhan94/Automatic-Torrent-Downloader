...
def helper():
    print(OutColors.DEFAULT + "\nSearch torrents from Kickass.to or press cntrl+c to quit ;)")
        ...
        if len(href) == 1:
            ...
            if torrent == 'Q' or torrent == 'q':
                ...
                if int(torrent) <= 0 or int(torrent) > len(href):
                    ...
                    #print(''+os.path.expandvars("%userprofile%")+'AppData\\Roaming\\uTorrent\\updates\\3.4.7_42330.exe')
                    fname = download_torrent(href[int(torrent)-1])
                    subprocess.Popen([''+os.path.expandvars("%userprofile%")+'\\AppData\\Roaming\\uTorrent\\updates\\3.4.7_42330.exe', fname], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
if __name__ == '__main__':
    try:                                    
        ...
        print('\nDeveloped by Shoaib khan. Thanks for using! \nwww.shoaibkhan.in')
